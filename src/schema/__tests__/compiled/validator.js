...
exports.SchemaDefinitionValidator = void 0;
// ── Implementation ──────────────────────────────────────────────────────────
    ...
    validate(schema) {
        ...
        schema.rpcCalls.forEach((call, i) => {
            ...
            if (!typeNames.has(call.inputType)) {
                errors.push(`rpcCalls[${i}] "${call.name}": inputType "${call.inputType}" ` +
                    `does not reference any defined type`);
            }
            // outputType must reference a known type
            if (!typeNames.has(call.outputType)) {
                errors.push(`rpcCalls[${i}] "${call.name}": outputType "${call.outputType}" ` +
                    `does not reference any defined type`);
            }
            // errorCodes in RPC call must reference known error codes
            call.errorCodes.forEach((code) => {
                if (!errorCodeNames.has(code)) {
                    errors.push(`rpcCalls[${i}] "${call.name}": errorCode "${code}" ` +
                        `does not reference any defined error code`);
                }
            });
        });
        // ── PATCH schema-004: duplicate name detection ──
        this.checkDuplicates(schema.rpcCalls.map((c) => c.name), 'rpcCalls', errors);
        this.checkDuplicates(schema.types.map((t) => t.name), 'types', errors);
        this.checkDuplicates(schema.errorCodes.map((e) => e.code), 'errorCodes', errors);
        return { valid: errors.length === 0, errors };
    }
    checkDuplicates(names, context, errors) {
        const seen = new Set();
        names.forEach((name) => {
            if (seen.has(name)) {
...
exports.SchemaDefinitionValidator = SchemaDefinitionValidator;
